...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  void _onChatInputChanged(ChatInputChanged event, Emitter<ChatState> emit) {
  ...
  Future<void> _onChatSubmitted(
    ...
    try {
      final reply = await sendChatMessage(config: config, message: trimmed);
  ...
  Future<void> _onDebateEvaluationRequested(
    ...
    if (config == null) {
      emit(state.copyWith(errorMessage: '평가에 필요한 토론 세션 정보가 없습니다. 다시 시작해주세요.'));
      return;
    }
  ...
  Future<void> _onDebateResetRequested(
    ...
    try {
      ...
      if (config == null) {
        emit(
          state.copyWith(isResetting: false, errorMessage: '세션 초기화에 실패했습니다.'),
        );
        return;
      }
  ...
  void _onChatErrorCleared(ChatErrorCleared event, Emitter<ChatState> emit) {
  ...
  ChatMessage _buildWelcomeMessage(DebateSessionConfig config) {
    return ChatMessage(
      ...
      text:
          '"${config.topic}"에 대해 이야기해 주세요. 먼저 주장을 말씀해주시면 그 논리를 바탕으로 반박을 이어가겠습니다.',
      isUser: false,
      createdAt: DateTime.now(),
    );
  }
